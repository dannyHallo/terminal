	...
	// 	{
		...
		// 		{
			...
			// 			float3 calculateLight(float3 rayOrigin, float3 rayDir, float rayLength, float3 originalCol, float2 uv) {
				...
				// 				// This is a hacky mess, TODoO: figure out a proper way to do this
...
Shader "Hidden/Atmosphere"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float densityAtPoint(float3 densitySamplePoint) {
				float heightAboveSurface = densitySamplePoint.y - atmosBottom;
				float height01 = clamp(heightAboveSurface / (atmosHeight - atmosBottom), 0, 1);
				float localDensity = exp(-height01 * densityFalloff) * (1 - height01);
				return localDensity;
			}
			...
			float opticalDepthBaked(float3 rayOrigin, float3 rayDir) {
				...
				// float height01 = saturate(height / (atmosphereRadius - planetRadius));
				float height01 = saturate(height / atmosHeight);
				// float uvX = 1 - (dot(normalize(rayOrigin - planetCentre), rayDir) * .5 + .5);
				float uvX = 1 - (dot(normalize(rayOrigin), rayDir) * .5 + .5);
				return tex2Dlod(_BakedOpticalDepth, float4(uvX, height01,0,0));
			}
			...
			float3 calculateLight(float3 rayOrigin, float3 rayDir, float rayLength, float3 originalCol, float2 uv) {
				...
				for (int i = 0; i < numInScatteringPoints; i ++) {
					sunRayLength = rayFlatAtmos(atmosBottom, atmosHeight, inScatterPoint, dirToSun);
					...
					localDensity = densityAtPoint(inScatterPoint);
					
					inScatteredLight += localDensity * transmittance * intensity * scatteringCoefficients * stepSize;
					inScatterPoint += rayDir * stepSize;
				}
			...
			float4 frag (v2f i) : SV_Target
			{
				// float testVal = atmosHeight / 10000;
				// The original col of this point
				const float epsilon = 0.0001;
				...
				// float dstToAtmosphere = 0;
				float dstThroughAtmosphere = min(rayFlatAtmos(atmosBottom, atmosHeight, rayOrigin, rayDir), sceneDepth);
				
				float colorFac = exp(-dstThroughAtmosphere* densityFalloff * 0.001) ;
				// return float4(colorFac, colorFac, colorFac, 1);

				float3 pointInAtmosphere = rayOrigin + rayDir * epsilon;
				float3 light = calculateLight(pointInAtmosphere, rayDir, dstThroughAtmosphere - epsilon * 2, originalCol, i.uv);
				return float4(light, 1);
