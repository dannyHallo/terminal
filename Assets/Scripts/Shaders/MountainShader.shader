...
Shader "ChinesePainting/MountainShader" 
{
    ...
    SubShader 
	{
		...
		{
			NAME "OUTLINE"
			// cull the front, leave us only the outline in this pass
			Cull Front
			CGPROGRAM
			...
			struct a2v 
			{
				// (x, y, z, w); w is for clipping
				// send pos to var vertex, etc.
			...
			struct v2f 
			{
				// sv stands for system value, recommanded for output from vertex shader
				// input to fragment shader, notice that the pos here is clipped
			...
			v2f vert (a2v v)
			{
				...
				// to add some bias by the normal direction
