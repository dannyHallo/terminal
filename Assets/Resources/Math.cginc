...
// Player underneath atmosohere
// Ray origin is inside atmos, so dstToAtmos = 0
float rayFlatAtmos(float atmosBottom, float atmosHeight, float3 rayOrigin, float3 rayDir) {
	float dstThroughAtmos;
	// dstThroughAtmos = min(maxFloat, (atmosHeight - rayOrigin.y) / abs(rayDir.y));
	float symmetryDistance = (atmosHeight - atmosBottom - rayOrigin.y) / abs(rayDir.y);
	dstThroughAtmos = log(symmetryDistance + 1) * 30;

	return dstThroughAtmos;
}
